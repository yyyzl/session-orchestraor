...
## MVP 缺口补录（按 2026-02-28 脑暴文档补齐）

1. 每轮策略输出强制字段：
   - 每轮都必须产出 `policy_decision`，且至少包含：
     - `decision_basis`（策略判定依据）
     - `decision_result`（判定结果，如 `continue_same_window` / `start_new_window`）
     - `action`（实际执行动作，如 `retry_current_step` / `start_new_window` / `mark_completed`）
2. 默认阈值落地：
   - `dev.unfinished_threshold_n = 1`（连续未完成阶段阈值默认值）。
   - 达到阈值时触发切窗策略。
3. 新窗口语义固定：
   - `start_new_window` 的实现语义为：`/new => 新 thread（同进程）`。
   - 策略事件中需显式记录该语义，便于审计与回放。
4. 统一提示词配置入口（V1 强制）：
   - 所有步骤提示词统一从单一配置中心读取（默认 `orchestrator_prompts.json`）。
   - 支持按任务类型覆盖，不允许在执行代码中散落硬编码模板。
5. `git提交` 指令生成约束：
   - `git提交` 步骤执行前必须先命中配置中心模板，再生成最终提交指令。
   - 回执/事件中需保留模板来源信息，证明命中配置中心而非临时硬编码。
